...
This chapter outlines the frameworks which have been used for the implementation
of this project and how they are put together into an application.

\subsection{Android}
The Android platform serves as the foundation of the whole implementation. The
Application has been built using existing building blocks such as Activities,
...
\begin{table}[htbp]
	...
	\begin{tabularx}{\textwidth}{llX}
		...
		\midrule
		MainActivity			& activity\_main.xml			& 	Startscreen of the application 	\\
		PollActivity			& activity\_poll.xml			& 	Activity which displays all 
																	the existing volls in a list	\\
		PollDetailActivity		& activity\_poll\_detail.xml	&	Activity which is used for
																	creating and editing votes		\\
		NetworkConfigActivity	& activity\_network\_config.xml	&	Allows to choose
...
\begin{table}[htbp]
	...
	\begin{tabularx}{\textwidth}{llX}
		...
		\midrule
		ConnectNetworkDialogFragment			& dialog\_join\_network.xml				& 	\\
		HelpDialogFragment						& dialog\_help.xml						& 	\\
		IdentificationWlanKeyDialogFragment		& dialog\_identification\_wlankey.xml	&	\\
		NetworkDialogFragment					& dialog\_network\_information.xml		&	\\
		NetworkInformationFragment				& fragment\_network\_information.xml	&	\\
		NetworkListFragment						& list\_item\_network.xml\footnote{In
		\emph{ListFragments} the layout for each row is applied} &
		\\
		NetworkOptionsFragment					& fragment\_network\_options.xml		& 	\\
		NFCFragment								& fragment\_nfc.xml						&	\\
		PollReviewFragment						& fragment\_poll\_review.xml			&	\\
		ResultChartDialogFragment				& dialog\_chart.xml						&	\\
		ResultChartFragment						& fragment\_result\_chart.xml			&	\\
		WaitForVotesFragment					& fragment\_wait\_for\_votes.xml		&	\\
		\bottomrule
	\end{tabularx}
	\end{minipage}
	\caption{Fragments}
...
\section{Enhancements to UniCrypt}
