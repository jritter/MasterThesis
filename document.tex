...
I would like to thank the E-Voting Group of the Bern University of Applied
Sciences for giving me the opportunity to realize this project. A special thanks
goes to my advisor Prof. Dr. Rolf Haenni who supported me throughout the whole
time during my master studies. A big thank you also goes to my fellow student
Philémon von Bergen with whom I worked closely during this master thesis. Coming
a long way from Germany for serving as an expert for this thesis, I would like
to mention my expert Stephan Neumann.
...
For the management of the available votes in the system (prepared as well as
completed votes), a small database structure has been implemented. The Android
...
votes, as well as an option to create a new vote. In the ``Vote Setup''
screen, the properties of the vote can be entered, namely the question of the vote and a
list of possible answers. The option \menu[,]{Allow blank ballots} can be
checked if voters should be allowed to cast a blank ballot.
...
according textbox at the top of the screen. This value defines how you will be
...
different network, you can use the button \keys{Advanced network configuration}. This screen
lets you choose all currently available network.
When choosing a network you should consider that all the participants need to be
allowed to connect to this network.

\section{Share the Session Parameters}
After selecting the network, a screen containing the voting session parameters
is displayed. Three parameters are required to connect to a voting session:
\begin{itemize}
...
\begin{itemize}
  ...
  \item \textbf{NFC tag:} The three parameters are written to a NFC tag. This
  tag can then be passed along the participants who can scan the tag by tapping
  it to the back of their device. Please note that NFC functionality is only
  available in devices of the latest generation.
\end{itemize}
...
the according textbox at the top of the screen. This value defines how you will be
represented in the voting session. Most likely you will want to enter your name
into this box. This value will be saved on the device and will be proposed in
upcoming sessions, but you can change this representation anytime you want to.
...
\begin{itemize}
  ...
  prompted to enter the password and the group number. These values are both
  displayed on the device of the administrator who established the voting
  session.
  \item \textbf{Advanced network configuration:} This option provides you with a
  list of currently available WLAN configuration. In order to join the voting
  ...
  tag which has been passed to you from the vote administrator. After tapping
  the NFC tag to the back of the device, the devices automatically connects to
  the voting session. In case the session takes place on a secure WLAN which is
  not known on the device, the user will be prompted to enter the WLAN key.
...
\keys{Next}. In the following screen, the administrator can approve all the
joined participants by clicking on the checkbox next to the allowed participants. Each
action automatically reflects immediately on all the joined devices. Please note
that the administrator can include or exclude himself in the electorate.
...
where all the participants sees a summary of the vote and its electorate. All
participants have to agree to this setup by clicking on the checkbox next to
...
\section{Voting}
In the voting screen, each participant can select the prefered option. After
confirming the choice, the vote is encrypted and cast. The following screen
summarises which participants cast their votes and which did not. The
administrator has the option to interrupt by clicking on \keys{Cancel vote}, in
...
After a successful tally, you will be redirected to the result screen where the
result of the vote is displayed along with some statistics. The vote is archived
and can be accessed anytime using the \keys{Vote archive} option on the main
screen. The administrator has the possibility to repeat the vote. This comes in
handy if several voting round are required to reach a decision.

\section{Vote Archive}
The vote archive accessible through the main screen lists all the past votes.
...
can then be started in the usual manner.
