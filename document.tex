...
text, plain text, signatures, etc. are treated as elements of algebraic
...
namely define the question and the possible options which can be chosen later
...
be started. The information about the choosen network as well as the generated
...
electorate) are correct. The vote can only be started by the administrator if
the whole electorate has agreed. If necessary, the administrator can go back and
do changes on the vote parameters.

The screen design of this phase from the administrator's perspective is
illustrate in \Vref{fig:admin_review} and from the voter's perspective in
...
which illustrates the vote casting state of all the participants. The screen
