...
group of people goes far back to the ancient greeks, who layed the groundwork of
...
\section{Outline}
Chapter \vref{cha:brw} gives an overview of the work on which this project has
...
Chapter \vref{cha:results} explains the resulting implementation in detail.
...
last parameter needed is a random generator $g$ of the group $G_q$. We can now derive the asymmetric key pair containing the secret key
...
\begin{enumerate}
  ...
  \item Compute $c=H(b||t)$, where $H$ represents a \emph{hashfunction} such
  as SHA-256
...
Let $(X,\oplus)$ and $(Y,\otimes)$ be two mathematical groups, $f:X
\rightarrow Y$ a one-way homomorphic function and $A=\{a_1, a_2, \ldots,
a_{n-1}, a_n\}$ a set of $n$ possible preimages. Assuming that we would like to prove
that the image $b=f(a_i)$ is indeed an image of the preimage $a_i \in A$ we
proceed as follows:
\begin{enumerate}
  \item Calculate $b_i=f(\frac{a_i}{a_j}) \in Y$ for $j=(1,\ldots,n)$
  \item Choose a uniformly random challenge $(c_1, c_2, \ldots, c_{i-1}, c_{i+1},\ldots,
  c_{n-1}, c_n) \in_R X$ for each preimage that we need to simulate
  \item Choose a uniformly random value signature $(s_1, s_2, \ldots, s_{i-1}, s_{i+1},\ldots,
  s_{n-1}, s_n) \in_R X$ for each preimage that we need to simulate
  \item Calculate the simulations $t_j=\frac{f(s_j)}{b_j^{c_j}} \in Y$ for
  $j=(1, 2, \ldots, i-1, i+1, \ldots, n-1, n)$
  \item Choose a uniformly random value $\omega_i \in_R X$
  \item Compute $t_i=f(\omega_i) \in Y$
  \item Compute $c=H(b_1||\ldots||b_n||t_1||\ldots||t_n)$, where $H$ represents
  a \emph{hashfunction} such as SHA-256
  \item Compute $c_i=c-\sum_{j=1, j \neq i}^n c_j \in X$
  \item Compute $s_i=\omega_i \oplus c_i \cdot a_i \in X$
...
of $n$ trustees as $T=\{t_1, \ldots, t_n\}$ and the set of $l$ voters as
$V=\{v_1, \ldots, v_l\}$. Furthermore, the role of an \emph{administrator} $A$
needs to be assigned to a participant. The process of these role assigments
strongly depends on the political structure of the organization which runs
this election and is considered as an administrative discussed and is hence not
...
each election. The details can be found in section \vref{sec:elgamal}. One
...
All the trustees execte a robust threshold key generation protocol as
discussed in section \vref{sec:thresholdsecretsharing}. The transcript of this
proctocol is posted on the bulletin board \myref{sec:bulletinboard}. The outcome
of this key generation protocol is an ElGamal public key $y$ which needs to be
communicated to all the voters $V$. 
