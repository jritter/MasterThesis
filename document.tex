...
footsepline, oneside, draft=off, listof=leveldown]{scrreprt}
...
\label{sec:evoting}
Electroing voting has been a field of research ever since the internet
...
properties mentioned above can be achieved by employing crypographic building
...
receipt which can be used to prove to another persion which option has been
...
crucial part which requires robustness is the bulletin board. To achive
robustness, the bulletin board should be replicated so that multiple consistent
instances are available. The CGS97 scheme doesn't specify in detail how this
should be achieved. A possible approach can be found in \cite{HL09}.
...
recipient. The recipient can then use its secret key to decrypt the AES key,
...
chosen. \Vref{fig:xmlstructuresingle} depicts the structure which is used for a
single encryption ballot vote, \Vref{fig:xmlstructuremulti} the structure used
for a multi encryption ballot vote.
...
devices is usually not as powerful as a hot spot which is designed for large
