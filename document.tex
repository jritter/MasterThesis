...
\begin{itemize}
  \item \textbf{Democracy:} Only eligible voters can a vote, and each voter can
  cast at most one ballot which will be included in the result.
  \item \textbf{Accuracy:} The result is derived only from \emph{all} valid
  votes as they were cast, i.e. cast votes can not be modified.
  \item \textbf{Universal verifiability:} Anyone can verify the correctness of
  the result.
  \item \textbf{Individual verifiability:} A voter can can verify that her vote
  is included in the result as she cast it.
  \item \textbf{Privacy:} Nobody can obtain information how a voter has
  voted.
  \item \textbf{Receipt-freeness:} The voter can not prove to anybody else how
  she voted.
  \item \textbf{Coercion-resistance:} Noone can force a voter to vote in a
  particular way, or prevent a voter to vote at all.
  \item \textbf{Robustness:} An attacker is not able to disrupt an election
